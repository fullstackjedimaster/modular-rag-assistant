...
router = APIRouter(prefix="/rag-clients", tags=["pdf-multimodal-ingest"])
...
def _safe_client_id(value: str) -> str:
    ...
    if not cleaned or any(ch not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_" for ch in cleaned):
        raise HTTPException(status_code=400, detail="Invalid rag_client_id")
    return cleaned


def _client_dir(rag_client_id: str) -> Path:
    return _source_docs_dir() / f"client_{_safe_client_id(rag_client_id)}"


def _jobs_dir(rag_client_id: str) -> Path:
    return _client_dir(rag_client_id) / "_jobs"


def _job_path(rag_client_id: str, job_id: str) -> Path:
    return _jobs_dir(rag_client_id) / f"{job_id}.json"
...
def _write_job(rag_client_id: str, job_id: str, data: dict) -> None:
    path = _job_path(rag_client_id, job_id)
...
def _read_job(rag_client_id: str, job_id: str) -> dict:
    path = _job_path(rag_client_id, job_id)
    if not path.exists():
...
def _run_job(
    ...
    job = _read_job(rag_client_id, job_id)
    job.update({"status": "running", "started_at": _now()})
    _write_job(rag_client_id, job_id, job)

    try:
        result = extract_pdf_multimodal(source_path, mode=mode, dpi=dpi, max_pages=max_pages)
        normalized_dir = _client_dir(rag_client_id) / "_normalized"
        ...
        job.update(
            {
                "status": "complete",
                "completed_at": _now(),
                "normalized_file": str(output_path.relative_to(_client_dir(rag_client_id))),
...
@router.post("/{rag_client_id}/docs/pdf-multimodal", status_code=202)
async def ingest_pdf_multimodal(
    ...
    client_dir = _client_dir(rag_client_id)
    client_dir.mkdir(parents=True, exist_ok=True)
    source_path = client_dir / filename
    ...
    _write_job(rag_client_id, job_id, job)
    background_tasks.add_task(_run_job, rag_client_id, job_id, source_path, mode, dpi, max_pages)
    return job


@router.get("/{rag_client_id}/docs/pdf-multimodal/jobs/{job_id}")
async def get_pdf_ingest_job(rag_client_id: str, job_id: str):
    return _read_job(rag_client_id, job_id)


@router.get("/{rag_client_id}/docs/pdf-multimodal/jobs")
async def list_pdf_ingest_jobs(rag_client_id: str):
    jobs_dir = _jobs_dir(rag_client_id)
    if not jobs_dir.exists():
        return {"rag_client_id": rag_client_id, "jobs": []}
    jobs = []
    for path in sorted(jobs_dir.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True):
        try:
            ...
            continue
    return {"rag_client_id": rag_client_id, "jobs": jobs}


@router.get("/{rag_client_id}/docs/pdf-multimodal/jobs/{job_id}/preview")
async def preview_pdf_ingest_job(
    ...
    path = (_client_dir(rag_client_id) / job["normalized_file"]).resolve()
    base = _client_dir(rag_client_id).resolve()
