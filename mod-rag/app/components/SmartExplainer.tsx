...
export function SmartExplainer({
    ...
    const effectiveId = subjectId ?? mac;
    ...
    useEffect(() => {
        const t: Telemetry = {};
        if (usecase.telemetry_keys?.length) {
            for (const key of usecase.telemetry_keys) {
                const v = attrs?.[key];
                if (v !== undefined && v !== null) t[key] = v as string | number;
            }
        }
        if (t.status === undefined && attrs?.status !== undefined) t.status = attrs.status as string;
        if (t.voltage === undefined && attrs?.voltage !== undefined) t.voltage = attrs.voltage as string | number;
        if (t.current === undefined && attrs?.current !== undefined) t.current = attrs.current as string | number;
        setTelemetry(t);
    }, [attrs, usecase]);
    ...
    const url = useMemo(() => {
        ...
        if (effectiveId) p.set("subject", effectiveId);
        for (const key of usecase.telemetry_keys) {
            const val = telemetry[key];
            if (val != null) p.set(key, String(val));
        }
        return `${explainPath}?${p.toString()}`;
    }, [query, telemetry, usecase, cfg.local_model, cfg.embed_model, cfg.provider, effectiveId, explainPath]);

    // === Stream hook ===
    const { streaming, banner, answer, progress, error, start, cancel, reset, contexts } = useStream({
        url,
        forceSSE: true,
        stallMs: 20000,
        heartbeatMs: 2000,
            ...
            try {
                const resp = await fetch(`${base}/diagnostics/heatmap`, {
                    ...
                    body: JSON.stringify({
                        ...
                        embed_model: cfg.embed_model, // ✅ dynamic per AIControls
                    }),
                });
                if (resp.ok) {
    ...
    return (
        <div className="bg-white dark:bg-gray-900 rounded-lg shadow p-4 space-y-6">
            ...
            <ExplanationPanel
                ...
                // ✅ pass new heatmap data down for visualization
